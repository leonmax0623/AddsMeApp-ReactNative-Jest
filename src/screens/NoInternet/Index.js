...
export default class NoInternetScreen extends Component {
	...
	refresh = () => NetInfo.fetch().then((state) => state.isConnected ? Storage.get('startScreen', (value) => this.props.navigation.navigate(value)) : {})
	render() {
		...
		return <View style={styles.wrapper}>
				<View style={s.warning}>
					...
					<Text style={[styles.text,styles.bold,styles.center]}>Ошибка соединения!</Text>
					<Text style={[styles.text,styles.mt20,styles.center]}>Отсутствует подключение к сети Интернет!</Text>
					<Text style={[styles.text,styles.mt5,styles.center]}>Без активного подключения к сети (сотовая связь или Wi-Fi), корректная работа приложения Adsme невозможна!</Text>
				</View>
				<TouchableOpacity style={{marginTop:10,alignItems:'center'}} onPress={() => this.refresh()}>
					<View style={styles.button}>
						<Text style={[styles.text,styles.white,styles.upper]}>Повторить попытку</Text>
					</View>
				</TouchableOpacity>
		</View>
	}
